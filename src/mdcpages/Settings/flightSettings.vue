<template>
  <div class="parent" v-if="file">
    <p class="mcd-s-2">Select Flight To Edit</p>
    <Dropdown
    ...
    <Button
      ...
      outlined
    />
    <p style="grid-row: 3" class="mcd-s-2">Member in selected Flight</p>
    <DataTable
    ...
    <p style="grid-row: 7" class="mcd-s-2">Edit Waypoints</p>
    <DataTable
      showGridlines
...
function deleteMember(i: number) {
  ...
  selctedFlight.value.units.map((n, i) => {
    (n.callsign = n.callsign),
      (n.tailNr = n.tailNr),
      (n.STN = n.STN.substring(0, n.STN.length - 1) + i);
    n.L16 = n.L16.substring(0, n.L16.length - 1) + i;
  });
}

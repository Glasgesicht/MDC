...
const toLatString = (lat: number) => {
  if (lat)
    return `${lat > 0 ? "N" : "S"} ${lat.toFixed(0).padStart(2, "0")}°${((lat % 1) * 60).toFixed(
      3
    ).padStart(6, "0")}`;
  return "";
}
...
const toLongString = (lon: number) => {
  if (lon)
    return `${lon > 0 ? "E" : "W"} ${lon.toFixed(0).padStart(3, "0")}°${((lon % 1) * 60).toFixed(
      3
    ).padStart(6, "0")}`;
  return "";
}
...
const minimumFuel = (steerpoint: number) => {
  ...
  routeFuel += fuelRange * parseFloat(calculateDistance(
              ...
              if (i !== 1) {
                routeFuel += ( parseInt(selectedFlight?.value.waypoints[i - 1].activity.substring(6,8)) / 60 / 60) * fuelEndurance
    + ( parseInt( selectedFlight?.value.waypoints[i - 1].activity.substring(3,5)) / 60) * fuelEndurance
    + ( parseInt( selectedFlight?.value.waypoints[i - 1].activity.substring(0,2))) * fuelEndurance;
              }
  }
  ...
  <div class="mdcpage">
    ...
    <div class="c36 parent" v-for="index in new Array(25).keys()">
      ...
      <div :class="`c3 ${index % 2 ? 'hg' : 'w'}`">
        {{ index === 0 ? takeoffTime(selectedFlight?.waypoints[index]?.tot, selectedFlight?.waypoints[index]?.activity) : hhmmss(selectedFlight?.waypoints[index]?.tot) }}
      </div>
      <div :class="`c4 ${index % 2 ? 'hg' : 'w'}`">
        {{
          selectedFlight?.waypoints[index]?.longitude && selectedFlight?.waypoints[index - 1]?.longitude
            ? calculateHeading(
              ...
              ).toFixed(0).padStart(3,"0")  + "°M / " + 
             parseInt(calculateDistance(
                selectedFlight?.waypoints[index - 1]?.latitude,
                selectedFlight?.waypoints[index - 1]?.longitude,
                selectedFlight?.waypoints[index]?.latitude,
                selectedFlight?.waypoints[index]?.longitude
      ...
      <div :class="`c3 ${index % 2 ? 'hg' : 'w'}`">
        {{ selectedFlight?.waypoints[index]?.groundspeed !== undefined ? selectedFlight?.waypoints[index]?.groundspeed?.toFixed(0) + " / " + selectedFlight?.waypoints[index]?.mach?.toFixed(2).replace("0.", ".") : ""}}
      </div>
      <div :class="`c3 ${index % 2 ? 'hg' : 'w'}`">
        {{
          selectedFlight?.waypoints[index]?.altitude
            ? selectedFlight?.waypoints[index]?.altitude?.toLocaleString('en-EN') + "ft"
            : ""
        }}
      </div>
      <div :class="`c3 ${index % 2 ? 'hg' : 'w'}`">
        {{ selectedFlight?.waypoints[index] ? parseInt(minimumFuel(index + 1)).toLocaleString('en-EN') : "" }}
      </div>
      <div :class="`c9 hr`">
        {{ selectedFlight?.waypoints[index]?.latitude !== undefined ? toLatString(selectedFlight?.waypoints[index]?.latitude) + " // " + toLongString(selectedFlight?.waypoints[index]?.longitude) : ""}}
      </div>
      <div :class="`c4 ${index % 2 ? 'hg' : 'w'}`">
        {{ selectedFlight?.waypoints[index]?.activity !== "00:00:00" ? index !== 0 ? selectedFlight?.waypoints[index]?.activity : "" : "" }}
      </div>
    </div>
    ...
    <div class="c36 parent" v-for="index in new Array(19).keys()">
      ...
      <div :class="`c3 ${(index + 81) < 97 ? (index % 2 ? 'hg' : 'w') : 'g'}`">{{ (index + 81) < 97 ? "" : "BULLS" }}</div>
      <div :class="`c5 ${index % 2 ? 'hg' : 'w'}`">{{ (index + 81) < 97 ? "" : getBullseyeName(index - 16) }}</div>
      <div :class="`c9 hr`">{{ (index + 81) < 97 ? "" : getBullseyeLocation(index - 16) }}</div>
      <div :class="`c3 ${(index + 81) < 97 ? index % 2 ? 'hg' : 'w' : 'g'}`">{{ (index + 81) < 97 ? "" : "" }}</div>

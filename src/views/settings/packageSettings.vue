<template>
  <div class="settings-parent" v-if="file">
    <div class="container">
      ...
      <div v-if="selectedPKG.name" class="box" style="max-width: 250px">
        <p>RAMROD (selected)</p>
        <Select :options="ramrods" v-model="selectedPKG.ramrod" editable class="m-5" />
      </div>
    </div>
    ...
    <div class="container" v-if="selectedPKG.name">
      <div class="box" style="max-width: 1100px">
        ...
        <DataTable :value="selectedPKG.flights" showGridlines edit-mode="cell" @rowReorder="onRowReorder"
          ...
          <Column headerStyle="width: 4.3rem">
            <template #body="{ index }"><Button icon="pi pi-copy" severity="" @click="copyFlight(index)"
                outlined /></template>
          </Column>
        </DataTable>
      </div>
  ...
  <div class="container">
    ...
    active: boolean;-->
      ...
      <div>
        <Button label="All" @click="selectedPKG.agencies.every((a) => (a.active = true))" style="margin: 4px" />
        <Button label="None" @click="selectedPKG.agencies.forEach((a) => (a.active = false))" style="margin: 4px" />
      </div>
    </div>
    <div v-if="selectedPKG.name" class="box">
      <p>Threat Classes</p>
      <DataTable :value="threats" style="min-width: 600px; max-width: 1100px" editMode="cell"
        @cell-edit-complete="onCellEditComplete" showGridlines>
      ...
      <div class="box">
        ...
        ></Column>-->
          <Column header="Action" field="action" style="width: 25%"><template #body="{ data }">{{ data.action
              }}</template><template #editor="{ data, field }">
              <InputText v-model="data[field]" />
            </template></Column>
        </DataTable>
      </div>

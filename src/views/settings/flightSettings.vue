...
<template>
  <div class="parent" style="max-width: 2000px">
    ...
    <div class="" v-if="file && getFlight.isActive">
      ...
      <Dropdown placeholder="select new callsign" v-if="!isCustomCalsign && getFlight.isActive" filter
      ...
      <div v-if="isCustomCalsign && getFlight" class="parent" style="max-width: 250px">
        <Input v-model="getFlight.callsign" @blur="flightStore.updateFligh()" />
        <InputNumber :min="1" :max="9" v-model="getFlight.callsignNumber" class="fixW"
          style="max-width: 75px; margin-left: 5px" @blur="useFlightStore().updateFligh()" />
      </div>
    </div>
    <div style="text-align: left; margin-top: 14px; width: 200px" class="parent" v-if="file && getFlight.isActive">
      <div class="">
  ...
  <TabView v-if="file && getFlight.isActive" style="margin-top: 10px">
    ...
    <TabPanel header="MISC Settings">
      <div class="miscSettings">
        ...
        <a>Enable Bullseye</a>
        <Dropdown :options="selectedPKG.bullseyes" option-value="wp" v-model:model-value="getFlight.misc.BullseyeWP">
  ...
  <div class="parent" v-if="file && getFlight.isActive">
    ...
    <div class="freqs">
      <div class="item5">
        <p>DEPART</p>
        <Dropdown :options="airports.filter((val) => val.map === theatre)" class="dropdown" severity="danger"
          option-label="NAME" v-model="getFlight.DEP" @change="(e: any) => {
            assignAirport('DEP', e.value);
          }
            " placeholder="select" />
        <Button style="grid-row: 15" v-if="getFlight.DEP.ICAO" icon="pi pi-times-circle" @click="deleteAirport('DEP')"
          text />
      </div>

      <div class="item5">
        <p>ARRIVE</p>
        <Dropdown :options="airports.filter((val) => val.map === theatre)" severity="danger" option-label="NAME"
          v-model="getFlight.ARR" @change="(e: any) => {
      ...
      <div class="item5">
        <p>ALTERNATE</p>
        <Dropdown :options="airports.filter((val) => val.map === theatre)" severity="danger" option-label="NAME"
          v-model="getFlight.ALT" @change="(e: any) => {
      ...
      <div class="item5">
        <p>TANKER</p>
        <Dropdown :options="selectedPKG.agencies
          .filter((ag) =>
            ['KC-135', 'KC135MPRS', 'KC130'].includes(ag.type)
          )
          .sort((a, b) => a.name.charCodeAt(0) - b.name.charCodeAt(0))
          " severity="danger" v-model="tanker" option-label="name" @change="(e: any) => {
            tanker = e.value;
          }
            " placeholder="select" />
        ...
        <DataTable showGridlines edit-mode="cell" :value="getFlight.comms.radio1" style="width: 450px">
          ...
          <Column header="Freq" field="freq" headerStyle="width: 2rem"
            ...
            <template #editor="{ data, index }">
              <Input class="fixW" v-model="getFlight.comms.radio1[index].freq" /></template>
          </Column>
          <Column header="Name" field="name" style="width: fit-content; padding: 2px 5px 2px 5px">
            <template #body="{ data }"> {{ data?.name }}</template>
            <template #editor="{ data, index }">
              <Input v-model="getFlight.comms.radio1[index].name" /></template>
          </Column>
          ...
          <Column header="Description" field="description" style="padding: 2px 5px 2px 5px">
            ...
            <template #editor="{ data, index }">
              <Input class="fixW" v-model="getFlight.comms.radio1[index].description" /></template>
          </Column>
      ...
      <div class="item">
        ...
        <DataTable showGridlines style="width: 450px" edit-mode="cell" :value="getFlight.comms.radio2">
          ...
          <Column header="Freq" field="freq" style="padding: 2px 5px 2px 5px">
            ...
            <template #editor="{ data, index }">
              <Input class="fixW" v-model="getFlight.comms.radio2[index].freq" /></template>
          </Column>
          <Column header="Name" field="name" style="padding: 2px 5px 2px 5px">
            <template #body="{ data }"> {{ data?.name }}</template>
            <template #editor="{ data, index }">
              <Input class="fixW" v-model="getFlight.comms.radio2[index].name" /></template>
          </Column>
          ...
          <Column header="Description" field="description" style="padding: 2px 5px 2px 5px">
            <template #body="{ data }"> {{ data?.description }}</template>
            <template #editor="{ data, index }">
              <Input class="fixW" v-model="getFlight.comms.radio2[index].description" /></template>
          </Column>
          <Column style="padding: 2px 5px 2px 5px; width: 20px">
